...
inline double calculateFreeSpacePathLoss(double transmittedPower, double gainTransmitter, double gainReceiver, double distance, double frequency) {
...
inline double calculateTwoRayGroundReflection(double transmittedPower, double gainTransmitter, double gainReceiver, double h_Transmitter, double h_Receiver, double distance) {
    double p_r = transmittedPower - 40*log10(distance) + 10*log10(gainTransmitter*gainReceiver*pow(h_Receiver*h_Transmitter,2));
    return p_r;
}

inline double calculateKnifeEdgeDiffraction(double transmittedPower, double wavelength, double t_to_obst, double r_to_obst, double h_Eff) {
...
inline double calculateEffectiveHeight(double t_to_obst, double r_to_obst, double h_Transmitter, double h_Receiver, double h_obstacle) {
    double h_Eff = h_obstacle - (h_Transmitter + t_to_obst * (h_Receiver - h_Transmitter) / (t_to_obst + r_to_obst));
    return h_Eff;
}

//{FSPL=Free Space path loss, TRGR=Two ray ground reflection, KED=Knife edge diffraction}
inline double checkSignalStrength(char *propagationModel,double transmittedPower,double gainTransmitter, double gainReceiver, double distance, double frequency,
        double h_Transmitter, double h_Receiver, double t_to_obst, double r_to_obst, double h_obstacle) {
    double result=0;
    if (strcmp(propagationModel, "FSPL") == 0){
        result = calculateFreeSpacePathLoss(transmittedPower, gainTransmitter, gainReceiver, distance, frequency);
    }
    else if (strcmp(propagationModel, "TRGR") == 0){
        result = calculateTwoRayGroundReflection(transmittedPower, gainTransmitter, gainReceiver, h_Transmitter, h_Receiver, distance);
    }
    else if (strcmp(propagationModel, "KED") == 0){
        const double c = 3.0e8;  // Speed of light in m/s
        double wavelength = c/frequency;
        //double h_obstacle=HeightMapLoader::findMaxHeightOnLine(x1, y1, x2, y2);
        double h_Eff=calculateEffectiveHeight(t_to_obst, r_to_obst, h_Transmitter, h_Receiver, h_obstacle);
        result = calculateKnifeEdgeDiffraction(transmittedPower, wavelength, t_to_obst, r_to_obst, h_Eff);
    }
    return result;
}
