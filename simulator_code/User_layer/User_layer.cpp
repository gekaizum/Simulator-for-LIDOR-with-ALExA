...
void UserLayer::algorithm(cMessage *msg){ // Handles incoming messages
        if(Start_Flag){
            ...
            // Take off Drone 1
            basicProtocolModule->set_velocity(1, 1, 1, 0, 0);
            basicProtocolModule->take_off(1, 1, 0, 0, 10);      // Drone 1 takes off to 10m altitude
            basicProtocolModule->move_to(1, 1, 150, 0, 10);     // Move Drone 1 forward until it reaches 150 meters
        ...
        basicProtocolModule->getDrone_position(1, 4, pos_array);
